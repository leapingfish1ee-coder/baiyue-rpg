...
实现状态：**Implemented**。当前能力与限制见[当前状态](current-state.md)，运行时增量见[阶段 3A 运行时契约](../specifications/phase-3a-runtime-contracts.md)。

- 发布 `graymane_boar`、`raw_hide`、Melee、Stealth 和狩猎 `TaskIntent`。
- 实现独立游戏事件随机域、一次性确定性遭遇和狩猎目标强制战斗。
- 实现一对一近战、击杀经验、掉落、跨战斗 HP 和持续自然恢复。
- 实现死亡 `60s` 等待、精确位置满 HP 复活、致死敌人恢复和 `5s RevivalGrace`。
- 复用既有事件引擎、任务、内容、存档与离线边界，不另建战斗 tick。
- 不加入 T2、工作站、Smithing、铜刃、防具、饰品或叙事。

实施依据为[阶段 3A 实施包](../engineering/phase-3a-t1-hunting-combat.md)和 [Decision-0007](../decisions/0007-t1-hunting-continuous-combat.md)。下一项直接工作是基于阶段 3A 的实际攻击、恢复和死亡结果封板阶段 3B，不提前增加 T2 或战斗装备兼容层。

### 阶段 3B：T2 与战斗装备生产

状态：**Planned**。阶段 3A 的实际战斗结果稳定后再封板。

- 增加营地工作站、Smithing、铜锭和铜刃。
- 增加 T2 边界威胁，并用真实路径、恢复和死亡结果校准装备收益。
- 只有兽皮获取与战斗属性已经可用后，才加入猎人护衣和潜行饰品。
- 不预建阶段 3A 尚未需要的武器替换、body/accessory 或工作站兼容层。

真源见[战斗、潜行与死亡](../requirements/combat-stealth.md)。

## 阶段 4：离线等价范围扩展

目标：在阶段 3A 已覆盖生活技能、敌人、战斗和死亡之后，扩展未来内容的离线等价范围与性能证据。
