...
- 产品界面不显示战斗、未实现技能、叙事、市场或多人入口。

### 阶段 2A 基础采集能力

- gameplay 内容层按绝对 `32×32 tiles` content cell 确定性放置 `wild_fiber` ambient candidate。placement hash 使用 seed、显式 `CONTENT_PLACEMENT_VERSION = 3`、prototype ID、营地 anchor 和绝对 cell 坐标，不修改 terrain payload；新增阶段 2C 内容不会移动已冻结节点，`GENERATOR_VERSION` 保持 `3`。
...
- 产品根现在显示五项技能、五种材料、四件工具候选、两种工具槽、三项配方、MaterialsMissing 和有符号离线物品净变化。不显示工作站、Smithing、铜刃、战斗装备、敌人、战斗、叙事或配方解锁。

### 本地存档、备份与离线

- `idb 8.0.3` 管理 IndexedDB。数据库使用 `meta`、`core`、`world_chunks` 和 `resume_claim` 四个无索引 store。
- `SAVE_SCHEMA_VERSION`、`GAME_RULES_VERSION` 和 `CONTENT_VERSION` 为 `4`；`DB_SCHEMA_VERSION` 保持 `1`。阶段 2B 存档不迁移，版本不匹配时保留原始导出和确认重置。
...
- 产品界面显示最近一次已提交离线报告，包括计入时间、丢弃时间、有符号非零 item delta、非空 skill XP 增量和揭露格数。生产消耗和产出使用同一净变化列表。

## 未实现

- 工作站生产、Smithing、铜刃、狩猎和配方解锁；
- 已发布五种材料和四件工具之外的物品、装备、掉落、容量和重量；
...
| 阶段 2C 产品 E2E 固定验证 rope 缺料、取得两份纤维、生产结算和 reload；强化工具制作与生产 continuous 未扩展为产品浏览器矩阵 | 强化工具门禁、swap、`+1000 bps` 来源、finite/continuous 和 reload 由固定 engine fixture 覆盖 |
| 仓库完整 `npm run build` 要求 `wasm-pack 0.15.0`，本机 PATH 当前只有 `0.14.0`，且 Homebrew Rust 缺少 WASM target | TypeScript strict、既有 generator-v3 WASM 的 Vite production build 和浏览器主路径可验证；本次未重新生成 release WASM |
