...
[存档与离线结算协议](../requirements/save-offline-protocol.md)已经接受 IndexedDB store 边界、gameplay worker 单写入者和 offline claim。精确 keyPath 与字段由存档专项在编码前补齐；当前仓库仍未实现该 schema。
...
事务 commit、dirty chunk records、自动保存和导入导出由[存档协议](../requirements/save-offline-protocol.md)定义，当前尚未实现。
